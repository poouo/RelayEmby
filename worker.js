...
function normalizeHostName(v) {
  ...
  if (!h) return "";
  try {
    if (/^https?:\/\//i.test(h)) h = new URL(h).host;
  } catch {}
...
function splitHostPort(host) {
  ...
  if (!s) return { hostname: "", port: "" };
  if (s.startsWith("[")) {
    const i = s.indexOf("]");
    return {
      hostname: i >= 0 ? s.slice(0, i + 1) : s,
      port: i >= 0 && s[i + 1] === ":" ? s.slice(i + 2) : "",
    };
  }
  const i = s.lastIndexOf(":");
  if (i > -1 && s.indexOf(":") === i) {
    const maybePort = s.slice(i + 1);
    if (/^\d+$/.test(maybePort)) {
      return { hostname: s.slice(0, i), port: maybePort };
    }
  }
  return { hostname: s, port: "" };
}
function getBaseDomain(env, request = null) {
...
function buildNodeOrigin(request, env, name) {
  const reqUrl = new URL(request.url);
  const baseDomain = getBaseDomain(env, request);
  const nodeName = String(name || "")
    .trim()
    .toLowerCase();
  if (!baseDomain || !nodeName) return reqUrl.origin;
  const { hostname: baseHost, port } = splitHostPort(baseDomain);
  const host = `${nodeName}.${baseHost}${port ? ":" + port : ""}`;
  return `${reqUrl.protocol}//${host}`;
}
function getNodeHostMatch(request, env) {
  const rawHost = request.headers.get("Host") || "";
  const { hostname } = splitHostPort(rawHost);
  const baseDomain = getBaseDomain(env, request);
  const { hostname: baseHost } = splitHostPort(baseDomain);
  ...
  if (!Validators.NAME_RE.test(sub)) return null;
...
const Validators = {
  NAME_RE: /^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])?$/i,
  SECRET_RE: /^[^\/?#\s]{0,128}$/,
  normalizeName(v) {
    return String(v || "")
      .trim()
  ...
  validateName(v) {
    ...
    if (!this.NAME_RE.test(name)) {
      return {
        ok: false,
        error:
          "name 非法:仅允许 a-z / 0-9 / -，长度 1~32，且不能以 - 开头或结尾",
      };
    }
...
const Database = {
  ...
  async getDnsConfig(env, request = null) {
    const kv = this.getKV(env);
    let cfg = {};
    if (kv) cfg = (await kv.get("sys:dns_config", { type: "json" })) || {};
    const inferredRecord =
      normalizeHostName(env.RELAYEMBY_BASE_DOMAIN || env.RELAY_BASE_DOMAIN || env.BASE_DOMAIN || "") ||
      (request ? normalizeHostName(request.headers.get("Host") || "") : "");
    return {
      token: String(cfg.token || env.CF_API_TOKEN || "").trim(),
      zoneName: String(cfg.zoneName || env.CF_ZONE_NAME || "").trim(),
      recordName: normalizeHostName(cfg.recordName || env.CF_RECORD_NAME || inferredRecord || ""),
      hasToken: !!String(cfg.token || env.CF_API_TOKEN || "").trim(),
      fromEnv: {
        token: !!String(env.CF_API_TOKEN || "").trim(),
        zoneName: !!String(env.CF_ZONE_NAME || "").trim(),
        recordName: !!String(env.CF_RECORD_NAME || "").trim(),
      },
    };
  },
  async setDnsConfig(env, cfg = {}) {
    const kv = this.getKV(env);
    if (!kv) throw new Error("D1未绑定! 请检查 EMBY_D1 / D1 / DB");
    const old = await this.getDnsConfig(env);
    const next = {
      token: String(cfg.token ?? old.token ?? "").trim(),
      zoneName: String(cfg.zoneName ?? old.zoneName ?? "").trim(),
      recordName: normalizeHostName(cfg.recordName ?? old.recordName ?? ""),
    };
    await kv.put("sys:dns_config", next);
    return {
      success: true,
      content: {
        token: next.token,
  ...
  async cfApi(env, method, path, body) {
    const cfg = await this.getDnsConfig(env);
    const token = cfg.token;
    if (!token) throw new Error("Cloudflare API Token 未配置");
  ...
  async getDnsRecordName(env, request = null) {
    const cfg = await this.getDnsConfig(env, request);
    const recordName = this.normalizeDnsHost(cfg.recordName || "");
    if (!recordName) {
      throw new Error("DNS 记录名未配置，请在后台 DNS 设置中填写，或配置 RELAYEMBY_BASE_DOMAIN");
    }
    return recordName;
  },
  guessZoneCandidates(recordName) {
    const parts = String(recordName || "").toLowerCase().split(".").filter(Boolean);
    const out = [];
    for (let i = 0; i <= Math.max(0, parts.length - 2); i++) {
      const z = parts.slice(i).join(".");
      if (z && !out.includes(z)) out.push(z);
    }
    return out;
  },
  async getZoneId(env, request = null) {
    const cfg = await this.getDnsConfig(env, request);
    const recordName = this.normalizeDnsHost(cfg.recordName || "");
    const candidates = [];
    const push = (v) => {
      const z = this.normalizeDnsHost(v || "");
      if (z && !candidates.includes(z)) candidates.push(z);
    };
    push(cfg.zoneName);
    for (const z of this.guessZoneCandidates(recordName)) push(z);
    if (!candidates.length) throw new Error("Zone 未配置，且无法从 DNS 记录名自动推断");
    ...
    throw new Error("未找到匹配的 Cloudflare Zone: " + candidates.join(" / "));
  },
  normalizeDnsHost(v) {
    let s = String(v || "").trim();
    if (!s) return "";
  ...
  async deleteDnsTypeRecords(env, type, recordName, request = null) {
  ...
  async upsertDnsRecord(env, type, content, request = null) {
    const recordName = await this.getDnsRecordName(env, request);
    ...
    let removedCname = 0;
    if (type === "A" || type === "AAAA") {
      removedCname = await this.deleteDnsTypeRecords(env, "CNAME", recordName, request);
    }
    if (replaceMode === "A" && type === "A") {
      await this.deleteDnsTypeRecords(env, "AAAA", recordName, request);
    } else if (replaceMode === "AAAA" && type === "AAAA") {
      await this.deleteDnsTypeRecords(env, "A", recordName, request);
    }
  ...
  async replaceDnsRecords(env, payload = {}, request = null) {
    const recordName = await this.getDnsRecordName(env, request);
    ...
    if (mode === "CNAME") {
      ...
      await this.deleteDnsTypeRecords(env, "A", recordName, request);
      await this.deleteDnsTypeRecords(env, "AAAA", recordName, request);
      await this.upsertDnsRecords(env, "CNAME", [cname], { ttl, proxied }, request);
    } else if (mode === "A") {
      if (!aList.length) throw new Error("A 模式至少填写一个 IPv4");
      await this.deleteDnsTypeRecords(env, "CNAME", recordName, request);
      await this.deleteDnsTypeRecords(env, "AAAA", recordName, request); // 关键：A时清空AAAA
      await this.upsertDnsRecords(env, "A", aList, { ttl, proxied }, request);
    } else if (mode === "AAAA") {
      if (!aaaaList.length) throw new Error("AAAA 模式至少填写一个 IPv6");
      await this.deleteDnsTypeRecords(env, "CNAME", recordName, request);
      await this.deleteDnsTypeRecords(env, "A", recordName, request); // 关键：AAAA时清空A
      await this.upsertDnsRecords(env, "AAAA", aaaaList, { ttl, proxied }, request);
    } else {
  ...
  async handleApi(request, env) {
    ...
    switch (data.action) {
      ...
      case "dns.config.set": {
        try {
          const out = await this.setDnsConfig(env, data.content || {});
          return new Response(JSON.stringify(out), {
            headers: { "Content-Type": "application/json;charset=utf-8" },
          });
        } catch (e) {
          return new Response(
            JSON.stringify({ error: e?.message || "保存 DNS 配置失败" }),
            {
              status: 500,
              headers: { "Content-Type": "application/json;charset=utf-8" },
            },
      ...
      case "dns.get": {
        try {
          const out = await this.getDnsStatus(env, request);
          return new Response(JSON.stringify(out), {
            headers: { "Content-Type": "application/json;charset=utf-8" },
          });
        } catch (e) {
      ...
      case "dns.replace": {
        try {
          const out = await this.replaceDnsRecords(env, data || {}, request);
          return new Response(JSON.stringify(out), {
            headers: { "Content-Type": "application/json;charset=utf-8" },
          });
        } catch (e) {
      ...
      case "node.compat.autofix": {
        ...
        const probeBase = buildNodeOrigin(request, env, name);
      ...
      case "checkStatus": {
        try {
          const buildProxyUrl = (n) => {
            ...
            if (!name) return "";
            return buildNodeOrigin(request, env, name);
          };
...
const ProxyHandler = {
          ...
          if (location.startsWith("/")) {
            ...
            if (loc.origin !== origin) {
              if (locHost === baseHost) {
                headers.set(
                  "Location",
                  origin + loc.pathname + loc.search + loc.hash,
                );
              } else {
                if (!nodeDirect) {
  ...
  async rewriteBodyLinks(
    ...
    const currentOrigin = this.nodeOrigin(
      { url: requestUrl, headers: new Headers({ Host: reqU.host }) },
      env,
      currentName,
    );
    ...
    for (const full of urls) {
      ...
      if (u.origin === origin || u.origin === currentOrigin) {
        continue;
      }
      if (nodeDirect) {
        map.set(full, full);
      ...
      if (curBaseHosts.has(h)) {
        map.set(full, origin + u.pathname + u.search + u.hash);
        continue;
      }
      const match = hostMap ? hostMap.get(h) || null : null;
      if (match) {
        const matchOrigin = this.nodeOrigin(
          { url: requestUrl, headers: new Headers({ Host: reqU.host }) },
          env,
          match.name,
        );
        map.set(full, matchOrigin + u.pathname + u.search + u.hash);
        continue;
      }
      map.set(full, origin + "/__raw__/" + encodeURIComponent(full));
    }
    let out = text;
...
<div id="app" style="display:none">
  ...
  <span class="top-ver">Emby 反代管理工具</span>
...
<div class="field-title"><span class="req">*</span> 二级域名前缀</div>
<input id="inName" placeholder="例如 movie、home-emby（a-z0-9-，1~32）">
<input id="inSec" type="hidden" value="">
...
<div id="cnameModal" class="modal glass">
  <h3>CNAME 更换</h3>
  <div class="field-title">Cloudflare API Token</div>
  <input id="cfApiToken" type="password" placeholder="用于 DNS 管理，可在后台保存">
  <div class="field-title">DNS 记录名</div>
  <input id="cfRecordName" placeholder="例如 emby.example.com；留空则使用基础域名">
  <div class="field-title">Zone 名称（可选）</div>
  <input id="cfZoneName" placeholder="例如 example.com；留空自动从记录名推断">
  ...
  <div class="btns">
    <button class="btn btn-g" onclick="App.closeAllModals()">关闭</button>
    <button class="btn btn-g" onclick="App.saveDnsConfig()">保存 DNS 配置</button>
...
const App = {
  ...
  getBaseDomain(){
    const configured = String(window.RELAYEMBY_BASE_DOMAIN || '').trim().toLowerCase().replace(/^\*\./, '').replace(/\.+$/, '');
    if (configured) return configured;
    const host = location.host.toLowerCase();
    const hostname = location.hostname.toLowerCase();
    if (hostname === 'localhost' || hostname.endsWith('.workers.dev')) return host;
    const parts = hostname.split('.').filter(Boolean);
    if (parts.length >= 3 && /^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])?$/.test(parts[0])) {
      const base = parts.slice(1).join('.');
      return base + (location.port ? ':' + location.port : '');
    }
    return host;
  },
  buildNodeHost(name){
    const base = this.getBaseDomain();
    const node = encodeURIComponent(String(name || '').trim().toLowerCase());
    return node && base ? node + '.' + base : '';
  },
  buildProxyUrl(n){
    const host = this.buildNodeHost(n?.name);
    return host ? (location.protocol + '//' + host) : '';
  },
  startAutoRefresh(){
  ...
  compatAutoFixEditor: async function () {
    ...
    if (!nameRaw) {
      this.toast("请先保存节点域名前缀，再执行一键修复。", "warn");
      return;
    }
...
applyBg(cfg){
  ...
  async loadDnsConfig() {
    ...
    const c = r.content || {};
    if ($("#cfApiToken")) $("#cfApiToken").value = c.token || "";
    if ($("#cfZoneName")) $("#cfZoneName").value = c.zoneName || "";
    if ($("#cfRecordName")) $("#cfRecordName").value = c.recordName || "";
  },
  async saveDnsConfig({ silent = false } = {}) {
    const content = {
      token: String($("#cfApiToken")?.value || "").trim(),
      zoneName: String($("#cfZoneName")?.value || "").trim(),
      recordName: String($("#cfRecordName")?.value || "").trim(),
    };
    const r = await API.req({ action: "dns.config.set", content });
    if (!r || !r.success) {
      if (!silent) this.toast((r && r.error) || "保存 DNS 配置失败", "error");
      return false;
    }
    if (!silent) this.toast("DNS 配置已保存", "success");
    return true;
  },
  async loadCnameStatus() {
    const r = await API.req({ action: "dns.get" });
    if (!r || !r.success) {
      $("#cnameCurrent").textContent = "--";
      if (r && r.error) this.toast(r.error, "warn");
      return;
    }
    if ($("#cfRecordName") && r.name) $("#cfRecordName").value = r.name || $("#cfRecordName").value;
    const cur = (Array.isArray(r.cname) && r.cname.length > 0) ? r.cname[0] : "";
    $("#cnameCurrent").textContent = cur || "--";
    $("#cnameValue").value = cur || "";
  },
  async saveCname() {
    const cfgOk = await this.saveDnsConfig({ silent: true });
    if (!cfgOk) return this.toast("请先填写并保存 Cloudflare DNS 配置", "warn");
...
async quickAddThirdParty(app, node, fullUrl) {
  ...
  renderList(){
    ...
    for(const n of arr){
     const fullUrl = this.buildProxyUrl(n);
...
pathTip.textContent = this.buildNodeHost(n.name);
...
v2.addEventListener('click', () => {
  ...
  this.copyText(proxyCopyUrl, '已复制代理地址');
});
const proxyCopyUrl = fullUrl;
...
<div class="project-links">
  ...
  <a href="https://github.com/poouo/RelayEmby" target="_blank" rel="noopener noreferrer">RelayEmby GitHub</a>
</div>
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (hostNodeName) {
      ...
      if (!nodeData) return new Response("Node Not Found", { status: 404 });
